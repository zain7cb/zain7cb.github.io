...
\[U(f,P) = \sum_P \sup_{[x_i,x_{i+1}]} f(t)(x_{i+1}-x_i),\ L(f,P) = \sum_P \inf_{[x_i,x_{i+1}]} f(t) (x_{i+1}-x_i).\]
...
A lot of the familiar properties of integrals still work, like additivity
and scaling. 

\textbf{Theorem.} If $A = A_1 \cup A_2$ where $A_1$ and $A_2$ only
overlap at possibly a finite number of curves, then
\[\iint_A f = \iint_{A_1} f + \iint_{A_2} f.\]
Also, if $A$ is some smooth curve contained in a rectangle $R$, and
$f$ is zero everywhere in $R$ except possibly at points in $A$,
then 
\[\iint_R f = 0.\]
