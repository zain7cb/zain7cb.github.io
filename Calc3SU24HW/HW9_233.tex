...
\[\iint_S \crl F \cdot \vec{n} d\sigma\]
where $F = (-y,x^2,z^3)$, and $S$ is the surface
\[x^2 + y^2 + z^2 = 1,\ -\frac{1}{2} \leq z \leq 1.\]
[Hint: by Stokes' theorem, $\iint_S \crl F \cdot \vec{n} d\sigma$ of any two surfaces that share the same boundary (with the same orientation) will have the same
